...
import {
  ...
  deletePricing,
  updatePricing,
} from "@/services/pricing-actions";

export default function TarifsClient({ initialPricing = [], initialError = "" }) {
  const router = useRouter();
  const permissions = useDashboardModulePermissions("pricing");
  ...
  const sortedPricing = useMemo(
    () => [...initialPricing].sort((a, b) => a.name.localeCompare(b.name)),
    [initialPricing]
  );
  ...
  const openEditModal = (item) => {
    ...
    setFormPrice(
      Number.isFinite(item.price) ? String(item.price) : item.price ?? ""
    );
    setFormError("");
    setIsEditOpen(true);
  };
  ...
  const handleCreate = async (event) => {
    ...
    const price = normalizePriceInput(formPrice);
    ...
    if (price === null) {
      setFormError("Veuillez saisir un tarif valide.");
      return;
    }
    ...
    try {
      const result = await createPricing({ name, price });
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deletePricing(pendingDelete.id);
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Gère les tarifs disponibles (ex: étudiant).
            </p>
          </div>
          ...
          <div className="px-6 py-5 border-b border-slate-200 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                {sortedPricing.length} tarifs disponibles
              </p>
            </div>
          </div>
        ...
        {isCreateOpen && permissions.canCreate ? (
          <Modal
            ...
            description="Renseigne le nom et le prix en DT."
        ...
        {isEditOpen && permissions.canUpdate ? (
          <Modal
            ...
            description="Mets à jour le nom et le prix en DT."
            onClose={() => (isSaving ? null : closeFormModals())}
          >
