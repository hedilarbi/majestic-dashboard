...
import {
  ...
  deleteShowType,
  updateShowType,
} from "@/services/show-types-actions";

export default function TypesSpectacleClient({
  initialShowTypes = [],
  initialError = "",
}) {
  ...
  const sortedShowTypes = useMemo(
    () => [...initialShowTypes].sort((a, b) => a.name.localeCompare(b.name)),
    [initialShowTypes]
  );
  ...
  const handleCreate = async (event) => {
    ...
    try {
      const result = await createShowType({ name });
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const result = await updateShowType({
        id: editingItem.id,
        name,
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteShowType(pendingDelete.id);
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Gère les types de spectacle disponibles.
            </p>
          </div>
          ...
          <div className="px-6 py-5 border-b border-slate-200 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="font-secondary text-lg font-semibold text-slate-900">
                Liste des types de spectacle
              </h2>
              <p className="text-sm text-slate-500">
                {sortedShowTypes.length} types disponibles
              </p>
            </div>
          </div>

          {sortedShowTypes.length === 0 ? (
            ...
            <form className="space-y-4" onSubmit={handleCreate}>
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="Ex : Comédie musicale"
