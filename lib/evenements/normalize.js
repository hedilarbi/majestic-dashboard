...
const getSessionOccupancy = (session) => {
  ...
  const sold =
    session.ticketsSold ?? session.soldSeats ?? session.bookedSeats ?? null;
  const capacity =
  ...
  if (Number.isFinite(available) && Number.isFinite(capacity) && capacity > 0) {
...
export const normalizeSessions = (payload, baseUrl = "") => {
  const items = Array.isArray(payload)
    ? payload
    ...
    .map((session) => {
      ...
      const eventDataRaw =
        ...
        (session?.eventId && typeof session.eventId === "object"
          ...
          : null);
      const eventData =
        eventDataRaw?.event || eventDataRaw?.data || eventDataRaw || null;
      const roomData =
        session?.room ||
      ...
      const eventName =
        ...
        session?.eventName ??
        "";
      const eventPosterSource =
        eventData?.poster ??
        eventData?.posterUrl ??
        eventData?.affiche ??
        eventData?.afficheUrl ??
        eventData?.image ??
        eventData?.imageUrl ??
        session?.eventPoster ??
        session?.poster ??
        "";
      const roomName =
        roomData?.name ?? roomData?.nom ?? session?.roomName ?? "";

      return {
        id,
        eventId,
        eventName,
        eventPoster: normalizePosterUrl(eventPosterSource, baseUrl),
        date: dateValue,
        sessionTime,
