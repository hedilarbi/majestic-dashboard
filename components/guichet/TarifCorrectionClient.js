  ...
  return (
    <article className="guichet-ticket-print-sheet relative mx-auto flex h-[123mm] min-h-[123mm] w-[79mm] min-w-[79mm] max-w-[79mm] flex-col rounded-md border border-slate-300 bg-white px-4 py-4 text-center text-black shadow-sm">
      <div className="flex flex-col items-center gap-0.5">
        ...
        <h2 className="text-[14px] font-black uppercase leading-tight">{eventName}</h2>
      </div>
      ...
      <p className="mt-1 text-center text-[12px] font-semibold">
        {ticket.pricingName || "Tarif"} : {Number(ticket.price).toFixed(3)} DT
      </p>
      ...
      <div className="mt-3 flex flex-col justify-center gap-1.5 text-[10px]">
      ...
      <div className="flex flex-col items-center gap-1">
        <p className="break-all font-mono text-[10px] font-semibold tracking-[0.03em] text-slate-700">
          {ticket.code}
        </p>
        {booking?.bookingNumber ? (
          <p className="text-[8px] text-slate-400">{booking.bookingNumber}</p>
        ) : null}
      </div>
      <p className="mt-auto text-center text-[8px] italic text-slate-400">
...
export default function TarifCorrectionClient() {
  ...
  // ── Step 3: Print ───────────────────────────────────────────────────────────
  const handlePrint = () => {
    if (!correctionResult?.ticket || !printRef.current) return;

    setIsPrinting(true);

    const resetPrintingState = () => {
    ...
    window.print();
    setTimeout(resetPrintingState, 3000);
  };
  ...
  return (
    <>
          ...
          <div className="space-y-5">
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => setStep(1)}
                className="flex-1 py-3.5 rounded-xl border border-slate-300 bg-slate-100 text-sm font-semibold text-slate-800 transition hover:bg-slate-200"
              >
                ← Retour
              </button>
              <button
      ...
      {/* Print area — hidden on screen, visible on print via globals.css guichet-print-root */}
      {correctionResult?.ticket ? (
        <div ref={printRef} className="guichet-print-root hidden">
